...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DYNAMIC DASHBOARD MEMBERSHIP CARD PRICING FOR FEMALE DATERS ---
...
let testimonialTimer = setInterval(() => {
    const totalSlides = document.querySelectorAll('.testimonial-card').length || 3;
    let nextIndex = (currentTestimonialIndex + 1) % totalSlides;
    ...
    if (userData) {
        ...
        document.querySelectorAll('.pricing-action button').forEach(btn => {
            ...
            if (card) {
                ...
                let isActive = false;
                if (isFemale) {
                    if (user.tier === 'Basic' && tierName === 'Basic Pass') {
                        isActive = true;
                    } else if ((user.tier === 'Premium' || user.tier === 'Elite') && tierName === 'Instadate Aura') {
                        isActive = true;
                    }
                } else {
                ...
                if (isActive) {
                    ...
                    if (isFemale && tierName === 'Basic Pass') targetTier = 'Basic';
                    if (isFemale && tierName === 'Instadate Aura') targetTier = 'Premium';

                    btn.textContent = `Switch to ${tierName}`;
                    btn.className = isFemale && tierName === 'Instadate Aura' ? 'btn btn-primary w-full btn-glow bg-cyan-glow' : 'btn btn-primary w-full btn-glow';
                    btn.setAttribute('onclick', `selectUpgradeTier('${targetTier}')`);
                }
            }
        });
        ...
        if (navProfile) {
            ...
            if (tierBadge) {
                const isFemale = isFemaleDater();
                const cleanTier = user.tier.toLowerCase();
                tierBadge.className = `tier-badge tier-badge-${cleanTier}`;
                const tierLabels = isFemale ? {
                    'Free': 'Free Member',
                    'Basic': 'Basic Pass',
                    'Premium': 'Instadate Aura',
                    'Elite': 'Instadate Aura'
                } : {
                    'Free': 'Free Member',
                    'Basic': 'Basic Member',
        ...
        if (megaConcierge) {
            ...
            const waText = `Hey Instadate Curation! 👋 My name is ${displayName}. I've successfully registered my account with the *${user.tier} Tier*. My ID is *${randomId}*. Let's fast-track my club matches! ✨`;
            megaConcierge.href = `${waBaseUrl}?text=${encodeURIComponent(waText)}`;
        ...
        if (megaConcierge) {
            const waBaseUrl = "https://wa.me/919999999999";
            const waText = `Hey Instadate Curation! 👋 I'm interested in joining the Instadate Social Club. Please guide me on fast-track membership onboarding! ✨`;
            megaConcierge.href = `${waBaseUrl}?text=${encodeURIComponent(waText)}`;
            megaConcierge.target = "_blank";
        }
    }
...
function updateDashboardPricing() {
    ...
    if (isFemale) {
        // 1. Basic Card -> Basic Pass
        const basicTitle = basicCard.querySelector('.tier-name');
        if (basicTitle) basicTitle.textContent = 'Basic Pass';
        
        const basicPriceAmount = basicCard.querySelector('.price-amount');
        if (basicPriceAmount) {
            basicPriceAmount.setAttribute('data-monthly', '99');
            basicPriceAmount.setAttribute('data-yearly', '69');
            // Check active state of yearly toggle to set correct text content
            const toggle = document.getElementById('price-toggle');
            const isYearly = toggle && toggle.classList.contains('toggled');
            basicPriceAmount.textContent = isYearly ? '69' : '99';
        }

        const basicFeatures = basicCard.querySelector('.pricing-features');
        if (basicFeatures) {
            basicFeatures.innerHTML = `
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>Only Verified Men Shown</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>College & Career Filters</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>Ghost Mode Visibility Controls</span>
                </div>
                <div class="feature-item text-muted">
                    <i data-lucide="x" class="feature-icon check-muted"></i>
                    <span>Elite Men Matching Pool Access</span>
                </div>
                <div class="feature-item text-muted">
                    <i data-lucide="x" class="feature-icon check-muted"></i>
                    <span>Premium Matching Priority</span>
                </div>
            `;
        }

        const basicBtn = basicCard.querySelector('.pricing-action button');
        if (basicBtn) {
            if (user && user.tier === 'Basic') {
                basicBtn.textContent = 'Active Plan';
                basicBtn.className = 'btn btn-outline w-full disabled';
                basicBtn.style.opacity = '0.7';
                basicBtn.setAttribute('onclick', 'event.preventDefault();');
            } else if (user) {
                basicBtn.textContent = 'Switch to Basic Pass';
                basicBtn.className = 'btn btn-primary w-full btn-glow';
                basicBtn.setAttribute('onclick', "selectUpgradeTier('Basic')");
            } else {
                basicBtn.textContent = 'Apply for Basic Pass';
                basicBtn.setAttribute('onclick', "openApplyModal('Basic')");
            }
        }

        // 2. Premium Card -> Instadate Aura
        const premiumTitle = premiumCard.querySelector('.tier-name');
        if (premiumTitle) {
            premiumTitle.textContent = 'Instadate Aura';
            premiumTitle.className = 'tier-name text-gradient-cyan';
        }
        
        const premiumPriceAmount = premiumCard.querySelector('.price-amount');
        if (premiumPriceAmount) {
            premiumPriceAmount.setAttribute('data-monthly', '199');
            premiumPriceAmount.setAttribute('data-yearly', '139');
            const toggle = document.getElementById('price-toggle');
            const isYearly = toggle && toggle.classList.contains('toggled');
            premiumPriceAmount.textContent = isYearly ? '139' : '199';
        }

        const premiumFeatures = premiumCard.querySelector('.pricing-features');
        if (premiumFeatures) {
            premiumFeatures.innerHTML = `
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-cyan"></i>
                    <span>Elite Men Only matching pool</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-cyan"></i>
                ...
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-cyan"></i>
                    <span>Premium Matching Priority desk</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-cyan"></i>
                    <span>Secure In-App Texting & Voice</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-cyan"></i>
                    <span>Offline Social Mixers Access</span>
                </div>
            `;
        }

        const premiumBtn = premiumCard.querySelector('.pricing-action button');
        if (premiumBtn) {
            const isPremiumActive = user && (user.tier === 'Premium' || user.tier === 'Elite');
            if (isPremiumActive) {
                premiumBtn.textContent = 'Active Plan';
                premiumBtn.className = 'btn btn-outline w-full disabled';
                premiumBtn.style.opacity = '0.7';
                premiumBtn.setAttribute('onclick', 'event.preventDefault();');
            } else if (user) {
                premiumBtn.textContent = 'Switch to Instadate Aura';
                premiumBtn.className = 'btn btn-primary w-full btn-glow bg-cyan-glow';
                premiumBtn.setAttribute('onclick', "selectUpgradeTier('Premium')");
            } else {
                premiumBtn.textContent = 'Apply for Instadate Aura';
                premiumBtn.className = 'btn btn-primary w-full btn-glow bg-cyan-glow';
                premiumBtn.setAttribute('onclick', "openApplyModal('Premium')");
            }
        }

        // 3. Elite Card -> Hide
        eliteCard.style.display = 'none';

        // 4. Center Grid
        pricingGrid.style.gridTemplateColumns = 'repeat(2, 1fr)';
        pricingGrid.style.maxWidth = '780px';
        pricingGrid.style.margin = '0 auto';
    } else {
        // Restores male standard basic card
        const basicTitle = basicCard.querySelector('.tier-name');
        if (basicTitle) basicTitle.textContent = 'Basic';
        
        const basicPriceAmount = basicCard.querySelector('.price-amount');
        if (basicPriceAmount) {
            basicPriceAmount.setAttribute('data-monthly', '199');
            basicPriceAmount.setAttribute('data-yearly', '139');
            const toggle = document.getElementById('price-toggle');
            const isYearly = toggle && toggle.classList.contains('toggled');
            basicPriceAmount.textContent = isYearly ? '139' : '199';
        }
        ...
        if (basicFeatures) {
            basicFeatures.innerHTML = `
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>100% ID-Verified Member Pool Access</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>3 Curated Matches per month</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>Secure In-App Texting & Voice</span>
                ...
                <div class="feature-item text-muted">
                    <i data-lucide="x" class="feature-icon check-muted"></i>
                    <span>Dedicated Matchmaking Assistant</span>
                </div>
                <div class="feature-item text-muted">
                    <i data-lucide="x" class="feature-icon check-muted"></i>
                    <span>Invite-only Offline Social Mixers</span>
                </div>
            `;
        }
        ...
        if (basicBtn) {
            if (user && user.tier === 'Basic') {
                ...
                basicBtn.textContent = 'Apply for Basic';
                basicBtn.setAttribute('onclick', "openApplyModal('Basic')");
            }
        }

        // Restores male standard premium card
        const premiumTitle = premiumCard.querySelector('.tier-name');
        if (premiumTitle) {
            premiumTitle.textContent = 'Premium';
            premiumTitle.className = 'tier-name text-gradient-pink';
        }
        
        const premiumPriceAmount = premiumCard.querySelector('.price-amount');
        if (premiumPriceAmount) {
            premiumPriceAmount.setAttribute('data-monthly', '399');
            premiumPriceAmount.setAttribute('data-yearly', '279');
            const toggle = document.getElementById('price-toggle');
            const isYearly = toggle && toggle.classList.contains('toggled');
            premiumPriceAmount.textContent = isYearly ? '279' : '399';
        }
        ...
        if (premiumFeatures) {
            premiumFeatures.innerHTML = `
                ...
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span><strong>8 Curated Matches</strong> per month</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>Secure In-App Texting, Voice & Video</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span><strong>Dedicated Human Matchmaker</strong> support</span>
                </div>
                <div class="feature-item">
                    <i data-lucide="check" class="feature-icon check-pink"></i>
                    <span>Invites to Offline Social Mixers (Tier-2)</span>
                </div>
            `;
        }
        ...
        if (premiumBtn) {
            if (user && user.tier === 'Premium') {
                ...
                premiumBtn.textContent = 'Switch to Premium';
                premiumBtn.className = 'btn btn-primary w-full btn-glow';
                premiumBtn.setAttribute('onclick', "selectUpgradeTier('Premium')");
            } else {
                premiumBtn.textContent = 'Apply for Premium';
                premiumBtn.className = 'btn btn-primary w-full btn-glow';
                premiumBtn.setAttribute('onclick', "openApplyModal('Premium')");
            }
        }
        ...
        // Elite Card -> Show
        eliteCard.style.display = 'flex';

        // Restore standard grid
        pricingGrid.style.gridTemplateColumns = 'repeat(3, 1fr)';
        pricingGrid.style.maxWidth = 'none';
        pricingGrid.style.margin = '0';
    }
...
function goToOnboardStage(stageNum) {
    ...
    if (stageNum === 1) {
        ...
        // Setup Pitch Card (Dynamic female subscriptions swap)
        ...
        if (pitchTitle && pitchPrice) {
            const isFemale = isFemaleDater();
            if (isFemale) {
                if (window._selectedOnboardTier === 'Premium' || window._selectedOnboardTier === 'Elite') {
                    pitchTitle.textContent = "Instadate Aura";
                } else {
                    pitchTitle.textContent = "Basic Pass";
                }
                const prices = { 'Basic': '99', 'Premium': '199', 'Elite': '199' };
                pitchPrice.textContent = prices[window._selectedOnboardTier] || '199';
            } else {
        ...
        if (benefitsContainer) {
            const isFemale = isFemaleDater();
            const tier = window._selectedOnboardTier || 'Premium';
            let html = '';
                ...
                if (tier === 'Basic') {
                    html = `
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                                <i data-lucide="shield-check"></i>
                            </div>
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Only Verified Men Shown</h4>
                                <p class="benefit-card-subtitle">Complete safety: only standard biometric Aadhaar-verified male profiles are presented.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">College & Career Filters</h4>
                                <p class="benefit-card-subtitle">Refined filters to find compatible profiles matching specific career and education circles.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Ghost Mode Visibility</h4>
                                <p class="benefit-card-subtitle">Complete control: browse privately and only reveal your profile to men you choose to vibe with.</p>
                            </div>
                        </div>
                    `;
                } else { // Instadate Aura (Premium/Elite)
                    html = `
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                                <i data-lucide="gem"></i>
                            </div>
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Elite Men Only</h4>
                                <p class="benefit-card-subtitle">Exclusivity check: connect strictly with high-tier verified Elite male circle members.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Everything From Basic Plan</h4>
                                <p class="benefit-card-subtitle">Includes verified men pool, college & career filters, and complete Ghost Mode controls.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Premium Matching Priority</h4>
                                <p class="benefit-card-subtitle">Double connection priority at our matchmaking concierge verification desk.</p>
                            </div>
                        </div>
                    `;
                }
                ...
                if (tier === 'Basic') {
                    html = `
                        <div class="benefit-item-card">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">100% ID-Verified Pool</h4>
                                <p class="benefit-card-subtitle">Access standard verified members with no bots or fake accounts.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">3 Curated Matches</h4>
                                <p class="benefit-card-subtitle">A handpicked selection matching your profile weekend style.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Secure In-App Channels</h4>
                                <p class="benefit-card-subtitle">Safe text & voice chat with mutual consent to protect details.</p>
                            </div>
                        </div>
                    `;
                } else if (tier === 'Elite') {
                    html = `
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Bespoke VIP Concierge</h4>
                                <p class="benefit-card-subtitle">Our premium, hand-guided matching pool with top-tier verification.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Unlimited Matching</h4>
                                <p class="benefit-card-subtitle">No monthly limits. Connect with verified matches as fast as you vibe.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">Elite Offline Mixers</h4>
                                <p class="benefit-card-subtitle">Invites to private styling circles, high-end dining, and VIP mixers.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card elite-item" style="border-color: rgba(0, 245, 255, 0.12);">
                            <div class="benefit-icon-badge bg-cyan-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title text-gradient-cyan">24/7 Matchmaker Access</h4>
                                <p class="benefit-card-subtitle">Direct hotline access to your dedicated elite concierge planner.</p>
                            </div>
                        </div>
                    `;
                    ...
                    html = `
                        <div class="benefit-item-card">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">8 Curated Vibe Matches</h4>
                                <p class="benefit-card-subtitle">Handpicked matches based on compatibility index & local cities.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Dedicated Human Matchmaker</h4>
                                <p class="benefit-card-subtitle">Direct 1-on-1 human support to filter bots & coordinate dates.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                            ...
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Offline Social Mixers</h4>
                                <p class="benefit-card-subtitle">Exclusive access to local high-vibe Bandra coffee & travel mixers.</p>
                            </div>
                        </div>
                        <div class="benefit-item-card">
                            <div class="benefit-icon-badge bg-pink-glow">
                                <i data-lucide="verified"></i>
                            </div>
                            <div class="benefit-text-box">
                                <h4 class="benefit-card-title">Priority Verification Badge</h4>
                                <p class="benefit-card-subtitle">Verified Aadhaar & Face Match to instantly double response rates.</p>
                            </div>
                        </div>
                    `;
                }
        ...
        const badgeLabel = document.getElementById('checkout-plan-badge');
        const priceValLabel = document.getElementById('payment-amount-display-val');
        
        const isFemale = isFemaleDater();
        const badges = isFemale ? {
            'Basic': 'Basic Pass',
            'Premium': 'Instadate Aura',
            'Elite': 'Instadate Aura'
        } : {
        ...
        if (badgeLabel) {
            badgeLabel.textContent = badges[window._selectedOnboardTier] || 'Premium Plan';
            if (window._selectedOnboardTier === 'Elite' || (isFemale && window._selectedOnboardTier === 'Premium')) {
                badgeLabel.style.background = 'var(--gradient-purple-cyan)';
            } else {
                badgeLabel.style.background = 'var(--gradient-pink-purple)';
            }
        }
        if (priceValLabel) {
            priceValLabel.textContent = priceDigits[window._selectedOnboardTier] || (isFemale ? '199' : '399');
        }
        ...
        // Initialize dynamic virtual credit card input formatting and 3D flip animation
        setTimeout(initCheckoutCardListeners, 50);
    } else if (stageNum === 7) {
        ...
        if (userData) {
            ...
            if (cardBadge) {
                ...
                if (user.tier === 'Elite' || (isFemale && user.tier === 'Premium')) {
...
function selectPresetAvatar(avatarKey) {
    ...
    updateOnboardPfpPreviewFromPreset(true);
}

function triggerOnboardPfpUpload() {
    const fileInput = document.getElementById('onboard-pfp-file-input');
    if (fileInput) fileInput.click();
}

function handleOnboardPfpFileChange(event) {
    const file = event.target.files[0];
    if (!file) return;

    if (file.size > 1.5 * 1024 * 1024) {
        alert("Please choose a profile picture smaller than 1.5MB.");
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        const preview = document.getElementById('onboard-pfp-preview-circle');
        if (preview) {
            preview.innerHTML = `<img src="${base64Data}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
        }

        const badge = document.getElementById('onboard-pfp-badge-label');
        if (badge) {
            badge.textContent = "Custom Photo";
            badge.className = "edit-pfp-badge badge-custom";
        }

        const customPfpInput = document.getElementById('onboard-profile-custom-pfp');
        if (customPfpInput) customPfpInput.value = base64Data;
    };
    reader.readAsDataURL(file);
}

function useOnboardArchetypePfp() {
    const customPfpInput = document.getElementById('onboard-profile-custom-pfp');
    if (customPfpInput) customPfpInput.value = "";
    updateOnboardPfpPreviewFromPreset(false);
}

function updateOnboardPfpPreviewFromPreset(preserveCustom) {
    const customPfpInput = document.getElementById('onboard-profile-custom-pfp');
    if (preserveCustom && customPfpInput && customPfpInput.value) return;

    const avatarKey = document.getElementById('onboard-profile-avatar')?.value || 'neon-cupid';
    const presets = {
        'neon-cupid': '💘',
        'mystic-dreamer': '✨',
        'cyber-flirt': '👾',
...
function handleOnboardProfileSubmit(event) {
    ...
    const gender = document.getElementById('onboard-profile-gender').value;
    ...
    const weekendStatus = document.getElementById('onboard-profile-weekend-status').value.trim();
    const avatar = document.getElementById('onboard-profile-avatar').value;
    const customPfp = document.getElementById('onboard-profile-custom-pfp')?.value || '';
    ...
    if (userData) {
        ...
        user.bio = bio;
        user.weekendStatus = weekendStatus;
        user.avatar = avatar;
...
function isFemaleDater() {
    ...
    // Form backup checks if currently onboarding
    const onboardGender = document.getElementById('onboard-profile-gender');
    const onboardInterest = document.getElementById('onboard-profile-interest');
    if (onboardGender && onboardGender.value === 'Female') return true;
    if (onboardInterest && onboardInterest.value === 'Men') return true;
    return false;
}
    ...
    try {
        ...
        setTimeout(() => {
            try {
                ...
                if (userData) {
                    const user = JSON.parse(userData);
                    user.tier = window._selectedOnboardTier || 'Premium';
                    localStorage.setItem('instadate_user', JSON.stringify(user));
                }
...
// Dynamic Virtual Credit Card interactive handlers & masks
function initCheckoutCardListeners() {
    const cardNumInp = document.getElementById('payment-card-num');
    const cardNameInp = document.getElementById('payment-card-name');
    const cardExpInp = document.getElementById('payment-card-exp');
    const cardCvvInp = document.getElementById('payment-card-cvv');
    
    const cardNumVis = document.getElementById('visual-card-num-display');
    const cardNameVis = document.getElementById('visual-card-name-display');
    const cardExpVis = document.getElementById('visual-card-exp-display');
    const cardCvvVis = document.getElementById('visual-card-cvv-display');
    const cardBrandVis = document.getElementById('visual-card-brand');
    const cardVisual = document.getElementById('visual-credit-card');
    
    // Prefill name from local storage if available
    const userData = localStorage.getItem('instadate_user');
    if (userData) {
        try {
            const user = JSON.parse(userData);
            if (user.name && cardNameInp) {
                cardNameInp.value = user.name;
                if (cardNameVis) cardNameVis.textContent = user.name;
            }
        } catch (err) {
            console.error("Error prefilling cardholder name:", err);
        }
    }
    
    // Event listeners for Live Syncing & Formats
    if (cardNumInp) {
        cardNumInp.addEventListener('input', (e) => {
            let val = e.target.value.replace(/\D/g, ''); // Allow digits only
            
            // Auto format grouping (xxxx xxxx xxxx xxxx)
            let formatted = '';
            ...
            // Live preview update
            if (cardNumVis) {
                cardNumVis.textContent = formatted || '•••• •••• •••• ••••';
            }
            
            // Brand detection
            if (cardBrandVis) {
                if (val.startsWith('4')) {
                    cardBrandVis.textContent = 'VISA';
                    cardBrandVis.style.textShadow = '0 0 10px rgba(0, 245, 255, 0.6)';
                    cardBrandVis.style.color = 'var(--primary-cyan)';
                } else if (val.startsWith('5')) {
                    cardBrandVis.textContent = 'MASTERCARD';
                    cardBrandVis.style.textShadow = '0 0 10px rgba(255, 46, 147, 0.6)';
                    cardBrandVis.style.color = 'var(--primary-pink)';
                } else if (val.startsWith('3')) {
                    cardBrandVis.textContent = 'AMEX';
                    cardBrandVis.style.textShadow = '0 0 10px rgba(155, 48, 255, 0.6)';
                    cardBrandVis.style.color = 'var(--primary-purple)';
                } else {
                    cardBrandVis.textContent = 'CARD';
                    cardBrandVis.style.textShadow = 'none';
                    cardBrandVis.style.color = 'var(--text-white)';
                }
            }
        });
    }
    ...
    if (cardNameInp) {
        cardNameInp.addEventListener('input', (e) => {
            ...
            if (cardNameVis) {
                cardNameVis.textContent = val || 'CLUB MEMBER';
            }
        });
    }
        ...
        cardExpInp.addEventListener('input', (e) => {
            ...
            if (val.length > 0) {
                ...
                if (val.length > 1) {
                    let second = val[1];
                    if (first === '1') {
                        // Month digit can only be 0, 1, 2 if first digit is 1
                        if (second !== '0' && second !== '1' && second !== '2') second = '2';
                    }
                    formatted += second;
                    formatted += '/';
            ...
            if (cardExpVis) {
                cardExpVis.textContent = formatted || 'MM/YY';
            }
        });
    }
    ...
    if (cardCvvInp) {
        cardCvvInp.addEventListener('input', (e) => {
            ...
            if (cardCvvVis) {
                cardCvvVis.textContent = val ? '•'.repeat(val.length) : '•••';
            }
        });
        
        // 3D Card Flip Focus Listeners
        cardCvvInp.addEventListener('focus', () => {
            if (cardVisual) cardVisual.classList.add('flipped');
        });
...
function runReviewTimelineAnimation() {
    ...
    // Animate high-tech scanner progress
    const progressFill = document.getElementById('scanner-progress-fill');
    const percentVal = document.getElementById('scanner-percentage-val');
    const dynamicText = document.getElementById('scanner-dynamic-text');
    
    const statusTexts = [
        "Analyzing profile matching nodes...",
        "Synchronizing weekend vibes...",
        "Evaluating chemical compatibility index...",
        "Consulting Bandra Matchmaking Board...",
        "Verifying checkout credentials...",
        "Finalizing verified member pass..."
    ];
    ...
    const scanInterval = setInterval(() => {
        ...
        if (percentVal) percentVal.textContent = `${Math.floor(progress)}%`;
        
        if (dynamicText) {
            const textIndex = Math.floor((progress / 100) * statusTexts.length);
            if (textIndex < statusTexts.length) {
                dynamicText.textContent = statusTexts[textIndex];
            }
        }
    }, intervalTime);
    ...
    if (list) {
        ...
        if (userData) {
            ...
            matchedList.forEach((m, idx) => {
                ...
                if (user.tier === 'Free') {
                    chatBtnHtml = `
                        <button class="btn btn-primary btn-sm font-semibold w-full" style="font-size: 0.72rem; padding: 6px 12px; flex-grow: 1; background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1); color: var(--text-muted);" onclick="closeCuratedMatchesModal(); handleLockedChatTrigger();">
                            Unlock Chat 🔒
                        </button>
                    `;
                } else {
...
function handleLockedChatTrigger() {
    alert("Unlock Premium Chats! 💬\n\nExplore for Free tier matches are view-only. Upgrade to any premium subscription (Basic, Premium, or Elite) to start talking with this partner instantly!");
    openSubscriptionModal();
}
...
function openSubscriptionModal() {
    ...
    if (userData) {
        ...
        // Dynamically adjust pricing and features for female users
        const basicPriceEl = document.querySelector('#sub-card-Basic .sub-plan-price');
        const basicFeaturesEl = document.querySelector('#sub-card-Basic .sub-plan-features');
        const basicTitleEl = document.querySelector('#sub-card-Basic .sub-plan-title');
        
        const premiumPriceEl = document.querySelector('#sub-card-Premium .sub-plan-price');
        const premiumFeaturesEl = document.querySelector('#sub-card-Premium .sub-plan-features');
        const premiumTitleEl = document.querySelector('#sub-card-Premium .sub-plan-title');
        const premiumDescEl = document.querySelector('#sub-card-Premium .sub-plan-desc');
        
        const eliteCard = document.getElementById('sub-card-Elite');
        const plansGrid = document.querySelector('.sub-plans-grid');
        
        if (isFemale) {
            // Update Basic Pass
            if (basicTitleEl) basicTitleEl.textContent = 'Basic Pass';
            if (basicPriceEl) basicPriceEl.textContent = '₹99 / mo';
            if (basicFeaturesEl) {
                basicFeaturesEl.innerHTML = `
                    <li><i data-lucide="check" class="text-pink"></i> Only verified men shown</li>
                    <li><i data-lucide="check" class="text-pink"></i> College/career preference filters</li>
                    <li><i data-lucide="check" class="text-pink"></i> Ghost mode</li>
                `;
            }
            
            // Update Instadate Aura
            if (premiumTitleEl) {
                premiumTitleEl.textContent = 'Instadate Aura';
                premiumTitleEl.className = 'sub-plan-title text-gradient-cyan';
            }
            if (premiumDescEl) premiumDescEl.textContent = 'VIP access with custom filters and direct channels.';
            if (premiumPriceEl) premiumPriceEl.textContent = '₹199 / mo';
            if (premiumFeaturesEl) {
                premiumFeaturesEl.innerHTML = `
                    <li><i data-lucide="check" class="text-cyan"></i> Elite men only</li>
                    <li><i data-lucide="check" class="text-cyan"></i> Everything from Basic Plan</li>
                    <li><i data-lucide="check" class="text-cyan"></i> Premium matching priority</li>
                `;
            }
            
            // Hide Elite Plan and adjust grid layout
            if (eliteCard) eliteCard.style.display = 'none';
            if (plansGrid) {
                plansGrid.style.gridTemplateColumns = 'repeat(2, 1fr)';
                plansGrid.style.maxWidth = '540px';
                plansGrid.style.margin = '24px auto 0';
            }
        } else {
            // Restore Male standard Basic
            if (basicTitleEl) basicTitleEl.textContent = 'Basic';
            if (basicPriceEl) basicPriceEl.textContent = '₹199 / mo';
            if (basicFeaturesEl) {
                basicFeaturesEl.innerHTML = `
                    <li><i data-lucide="check" class="text-pink"></i> 100% ID-Verified member pool</li>
                    <li><i data-lucide="check" class="text-pink"></i> 3 Curated Matches / mo</li>
                    <li><i data-lucide="check" class="text-pink"></i> Secure text & voice chat</li>
                `;
            }
            
            // Restore Male standard Premium
            if (premiumTitleEl) {
                premiumTitleEl.textContent = 'Premium';
                premiumTitleEl.className = 'sub-plan-title text-gradient-pink';
            }
            if (premiumDescEl) premiumDescEl.textContent = 'Our flagship matchmaker-driven experience for conscious dating.';
            if (premiumPriceEl) premiumPriceEl.textContent = '₹399 / mo';
            if (premiumFeaturesEl) {
                premiumFeaturesEl.innerHTML = `
                    <li><i data-lucide="check" class="text-pink"></i> 8 Curated Matches / mo</li>
                    <li><i data-lucide="check" class="text-pink"></i> Dedicated Human Matchmaker</li>
                    <li><i data-lucide="check" class="text-pink"></i> Invites to offline mixers (Tier-2)</li>
                `;
            }
            
            // Show Elite Plan and restore standard layout
            if (eliteCard) eliteCard.style.display = 'flex';
            if (plansGrid) {
                plansGrid.style.gridTemplateColumns = 'repeat(3, 1fr)';
                plansGrid.style.maxWidth = 'none';
                plansGrid.style.margin = '24px 0 0';
            }
        }
        
        // Reset sub cards active styles and restore original classes
        ...
        document.querySelectorAll('.sub-plan-card button').forEach(btn => {
            ...
            btn.style.opacity = '1';
            if (btn.id === 'sub-btn-Premium') {
                btn.textContent = isFemale ? 'Select Aura' : 'Select Premium';
                btn.className = isFemale ? 'btn btn-primary w-full btn-glow mt-4 bg-cyan-glow' : 'btn btn-primary w-full btn-glow mt-4';
            } else if (btn.id === 'sub-btn-Basic') {
                btn.textContent = 'Select Basic';
                btn.className = 'btn btn-outline w-full mt-4';
            } else if (btn.id === 'sub-btn-Elite') {
                btn.textContent = 'Select Elite';
                btn.className = 'btn btn-outline w-full mt-4';
            } else {
        ...
        // Highlight active sub card and truly disable its button
        let activeTier = user.tier;
        // Map Elite tier for female users to Premium card so it highlights Aura
        if (isFemale && activeTier === 'Elite') {
            activeTier = 'Premium';
        }
        const activeCard = document.getElementById(`sub-card-${activeTier}`);
        if (activeCard) {
            if (activeTier === 'Elite' || (isFemale && activeTier === 'Premium')) {
                activeCard.classList.add('active-elite');
            } else {
                activeCard.classList.add('active');
            }
            
            const activeBtn = document.getElementById(`sub-btn-${activeTier}`);
            if (activeBtn) {
...
function selectUpgradeTier(tier) {
    ...
    // Save target tier globally and go directly to checkout for existing members.
    window._selectedOnboardTier = tier;
...
const liveEventsData = [
    {
        ...
        title: "Acoustic & Coffee Mixer 🎸",
        location: "Secret Garden Cafe, Bandra West",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [19.1025, 72.8270],
        title: "Secret Rooftop Sunset Soiree 🌇",
        location: "Sea-view Deck, Juhu",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [18.9220, 72.8346],
        title: "Speakeasy Private Cocktail Session 🍸",
        location: "Private Speakeasy Cellar, Colaba",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [18.9200, 72.8300],
        title: "Alibaug Yacht Weekend Escape ⛵",
        location: "Gateway Marina, Colaba",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [28.5961, 77.2175],
        title: "Concert & Wine Night 🍷",
        location: "Secret Courtyard, Chanakyapuri, Delhi NCR",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [12.9785, 77.64],
        title: "High-Fi Vinyl Listening 🎵",
        location: "Analog Glassroom, Indiranagar, Bangalore",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [15.0118, 74.015],
        title: "Off-Grid Beachside Campfire 🔥",
        location: "Secret Beachfront, South Goa",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [28.4595, 77.0726],
        title: "Skyline Golf & Cocktails ⛳",
        location: "DLF Club Roof, Gurgaon, Delhi NCR",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [17.4156, 78.4347],
        title: "Secret Villa Jazz Piano 🎹",
        location: "Aesthetic Heritage Villa, Banjara Hills, Hyderabad",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [12.9352, 77.6244],
        title: "Founders Craft Brewery Mixer 🍺",
        location: "Rooftop Microbrewery, Koramangala, Bangalore",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [12.8398, 80.245],
        title: "ECR Beach Acoustic Dinner 🎸",
        location: "Coastal Lawn, ECR Road, Chennai",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [22.5487, 88.3517],
        title: "Art Deco Poetry & Single Malts 🥃",
        location: "Private Lounge, Park Street, Kolkata",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [26.9855, 75.8513],
        title: "Royal Fort Sunset Soiree 🌇",
        location: "Rooftop Terrace, Nahargarh Fort, Jaipur",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [19.0176, 72.8561],
        title: "Gateway Speakeasy Yacht Regatta ⛵",
        location: "Colaba Gateway Marina, Mumbai",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [18.5362, 73.894],
        title: "Greenhouse Gin & Tech Mixer 🍸",
        location: "Glass Greenhouse, Koregaon Park, Pune",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [28.6015, 77.2198],
        title: "Lutyens Champagne Art Gallery 🍾",
        location: "Premium Gallery, Lutyens, Delhi NCR",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [15.5164, 73.7632],
        title: "Candolim Yacht & Beach Escapade ⛵",
        location: "Luxury Yacht, Candolim Beach, Goa",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [18.5204, 73.8567],
        title: "Vineyard Classical Harp & Wine 🎻",
        location: "Vineyard Estate Hills, Pune",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [22.5626, 88.3639],
        title: "Heritage Walks & Coffee Chats ☕",
        location: "Heritage Bookstore Cafe, College Street, Kolkata",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [30.7333, 76.7794],
        title: "Secret Garden Acoustic Covers 🎸",
        location: "Lush Courtyard Lawn, Sector 10, Chandigarh",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [28.495, 77.089],
        title: "CyberCity Corporate speed-dating ⚡",
        location: "High-Fi Lounge, CyberCity, Gurgaon",
        color: "#9B30FF",
        radius: 280
    },
    {
        coords: [17.4246, 78.4124],
        title: "Lake Lawn Stargazing & Acoustics ✨",
        location: "Lakeside Lawn, Jubilee Hills, Hyderabad",
        color: "#00F5FF",
        radius: 280
    },
    {
        coords: [9.5931, 76.4225],
        title: "Kumarakom Houseboat Match Getaway ⛵",
        location: "Kumarakom Lake Resort Marina, Kochi",
        color: "#FF2E93",
        radius: 280
    },
    {
        coords: [26.9124, 75.7873],
        title: "Heritage Palace Sitar-Guitar Fusion 🎸",
        location: "Heritage Palace Lawn, C-Scheme, Jaipur",
        color: "#9B30FF",
        radius: 280
    }
];
const coupleMatchesData = [
    ...
    {
        name: "Priya & Sameer",
        coords: [28.5244, 77.2066],
        city: "Delhi NCR",
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Priya & Sameer</h4>
                <p class="popup-subtitle">Saket Books, Delhi</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
        `,
        color: '#FF2E93'
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Shruti & Akhil</h4>
                <p class="popup-subtitle">Secret Garden, Bandra</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
        `,
        color: '#9B30FF'
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Kavita & Nikhil</h4>
                <p class="popup-subtitle">Venture Hub, Bangalore</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
        `,
        color: '#00F5FF'
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Tanvi & Aditya</h4>
                <p class="popup-subtitle">Western Ghats Camp, Pune</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Sneha & Rahul</h4>
                <p class="popup-subtitle">Street Food Drive, Hyderabad</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Natasha & Vikram</h4>
                <p class="popup-subtitle">Yacht Escape, Goa</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Diya & Rohit</h4>
                <p class="popup-subtitle">SaaS Office Roof, Gurgaon</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Meera & Ashwin</h4>
                <p class="popup-subtitle">ECR Beach Walk, Chennai</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Riddhi & Sarthak</h4>
                <p class="popup-subtitle">Bookstore Cafe, Kolkata</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Simran & Gurpreet</h4>
                <p class="popup-subtitle">Running Club, Chandigarh</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Ameya & Gautham</h4>
                <p class="popup-subtitle">Lakeside Kayaking, Kochi</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Pooja & Varun</h4>
                <p class="popup-subtitle">Heritage Fort, Jaipur</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
    ...
    {
        name: "Neha & Armaan",
        coords: [18.922, 72.8346],
        city: "Mumbai",
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Neha & Armaan</h4>
                <p class="popup-subtitle">Speakeasy Lounge, Mumbai</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Kritika & Varun</h4>
                <p class="popup-subtitle">Morning Running, Gurgaon</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Rhea & Sid</h4>
                <p class="popup-subtitle">Concert Buddy, Bangalore</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
    ...
    {
        name: "Kiara & Ishaan",
        coords: [18.536, 73.893],
        city: "Pune",
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Kiara & Ishaan</h4>
                <p class="popup-subtitle">Philosophy Cafe, Pune</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    {
        name: "Maya & Vivek",
        coords: [17.415, 78.435],
        city: "Hyderabad",
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Maya & Vivek</h4>
                <p class="popup-subtitle">Startup Desk, Hyderabad</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Aisha & Kabir</h4>
                <p class="popup-subtitle">Beach Bonfire, Goa</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #00F5FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #00F5FF; box-shadow: 0 0 8px #00F5FF;"></span> Couple Met
                </div>
            </div>
    ...
    {
        name: "Shruti & Siddharth",
        coords: [28.45, 77.06],
        city: "Gurgaon",
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Shruti & Siddharth</h4>
                <p class="popup-subtitle">Single-origin Brews, Gurgaon</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #FF2E93; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #FF2E93; box-shadow: 0 0 8px #FF2E93;"></span> Couple Met
                </div>
            </div>
    ...
    },
    {
        ...
        popupText: `
            <div class="map-popup-card">
                <h4 class="popup-title">Tara & Karthik</h4>
                <p class="popup-subtitle">ECR Drives, Chennai</p>
                <div class="popup-status" style="background: rgba(155, 48, 255, 0.15); color: #9B30FF; border-color: rgba(155, 48, 255, 0.25);">
                    <span class="popup-dot" style="background-color: #9B30FF; box-shadow: 0 0 8px #9B30FF;"></span> Couple Met
                </div>
            </div>
...
function initInteractiveMap() {
    ...
    // Initialize all live event markers and circles
    liveEventMarkers = [];
    liveEventCircles = [];
    
    liveEventsData.forEach((evt, idx) => {
        ...
        marker.bindPopup(`
            <div class="map-popup-card">
                <h4 class="popup-title">${evt.title}</h4>
                <p class="popup-subtitle">${evt.location}</p>
                <div class="popup-status" style="background: rgba(255, 46, 147, 0.1); color: ${evt.color}; border-color: rgba(255, 46, 147, 0.2);"><span class="popup-dot" style="background-color: ${evt.color};"></span> Mixer Active</div>
            </div>
        `, {
            closeButton: false,
            autoClose: false,
        ...
        liveEventMarkers.push(marker);

        const circle = L.circle(evt.coords, {
            color: evt.color,
            fillColor: evt.color,
            fillOpacity: 0.06,
            radius: evt.radius,
        ...
        liveEventCircles.push(circle);
    });

    // Assign the first one to the legacy single references for backwards compatibility
    if (liveEventMarkers.length > 0) {
        liveEventMarker = liveEventMarkers[0];
        liveEventCircle = liveEventCircles[0];
        // Open the first event's popup by default
        liveEventMarkers[0].openPopup();
    }
...
function switchMapLayer(layer) {
    ...
    if (layer === 'event') {
        ...
        // Show/Hide Markers
        liveEventMarkers.forEach(m => showLayer(m));
        liveEventCircles.forEach(c => showLayer(c));
        coupleMarkers.forEach(m => hideLayer(m));
        memberMarkers.forEach(m => hideLayer(m));
        
        // Get active event index (default to 0 if none active)
        let activeIdx = 0;
        const activeCard = document.querySelector('#sidebar-events-list .couple-map-card.active');
        if (activeCard) {
            const cardId = activeCard.id;
            activeIdx = parseInt(cardId.split('-')[2]) || 0;
        }

        // Fly map back to active live coordinates
        leafletMapInstance.closePopup();
        const targetCoords = liveEventsData[activeIdx].coords;
        leafletMapInstance.flyTo(targetCoords, 15, { duration: 1.5 });
        setTimeout(() => {
            if (liveEventMarkers[activeIdx] && leafletMapInstance.hasLayer(liveEventMarkers[activeIdx])) {
                liveEventMarkers[activeIdx].openPopup();
            }
        }, 1500);
        ...
        // Show/Hide Markers
        liveEventMarkers.forEach(m => hideLayer(m));
        liveEventCircles.forEach(c => hideLayer(c));
...
// Focus on a specific live event when they click a sidebar card
function focusLiveEvent(index) {
    if (!leafletMapInstance || !liveEventMarkers[index]) return;
    
    leafletMapInstance.closePopup();
    leafletMapInstance.flyTo(liveEventsData[index].coords, 15, { duration: 1.5 });
    
    setTimeout(() => {
        if (liveEventMarkers[index]) {
            liveEventMarkers[index].openPopup();
        }
    }, 1500);
    
    // Highlight active card
    const cards = document.querySelectorAll('#sidebar-events-list .couple-map-card');
